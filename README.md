...
2. Update `main.tf` for the `azurerm_federated_identity_credential` source to use the `subject` matching your Github repository.
3. After installing Azure CLI and logging in with `az login`, run `azure account show` to get your subscription information.
4. In the output, fetch the `"id"` value.
5. Create your `.env` file: `cp .env.template .env`.
6. Paste your subscription ID into the `.env` file.
7. Set up terraform: `source .env && terraform init`.
8. Deploy your web app: `source .env && terraform apply`.
    * DON'T FORGET TO TEARDOWN SO YOU DON'T SPEND MONEY.
9. Create the necessary Github Actions secrets in your Github repository settings, using the terraform outputs and container registry credentials in Azure Portal.
10. Update `.github/workflows/workflow.yaml` to use the login server shown by terraform and the username and password found in Azure Portal for your container registry.
11. Update `.github/workflows/workflow.yaml` to use your `app-name` you set in step 1.
